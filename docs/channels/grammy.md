...
- **Streaming:** `channels.telegram.streamMode` selects `edit` (message edits) or `partial` (sendMessageDraft in private topic chats, Bot API 9.3+). `off` disables streaming. Separate from channel block streaming.
